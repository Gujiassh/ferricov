...
- `compat/upstream/installed-tree.sh` (optional directory retention)
...
Accept as a bounded M0 Oracle-reference capture batch:

- all 13 planned `INST-*` identities now bind wave-2 capture artifacts under
  `compat/installation/wave2/`;
...
- Installation contract extended with wave-2 capture bindings and directory
  companion evidence
...
3. Staged DESTDIR install yields 320 payload files and 0 symlinks; compiled
   paths retain PREFIX without DESTDIR embedding.
...
7. Induced install-loop failure leaves a non-transactional partial payload
   (18 files observed) without rollback.
...
- `python3 -m unittest compat.installation.test_contract` (42 tests)
- Contract regenerate/validate against pinned upstream checkout
- Reverse mutation coverage retained and extended for wave-2 bindings
- `compat/verify.py --skip-oracle` and installation schema/contract checks
