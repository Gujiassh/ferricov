...
| Oracle resource observation | 13 controlled profiles bind exact source-scoped input shapes, branch/MC/DC summaries and streams, raw metrics, cleanup, and runtime identity; host-deadline/nonzero failures retain canonical diagnostics when storage is writable, retention failure cannot bypass cleanup, and the successful tree rejects extras/symlinks; 13/13 exact inputs are accepted without selecting a Ferricov limit or compatibility/performance claim |
