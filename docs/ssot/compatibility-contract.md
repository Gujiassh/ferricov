...
| coverage-model algebra decisions | Oracle-only model contract for set algebra, parse-write-parse, and repeated-close decisions | CORE-001…008 complete; bounded CORE-009 property/fuzz implementation awaiting Critical acceptance | 157 pinned Oracle cases bind 7 rows (`M1-MD-010..014`, `017`, `019`); deterministic stable property tests and nine budgeted libFuzzer smoke targets add no differential evidence; `M1-MD-020`, `M1-TF-063`, and `M1-TF-064` remain blocked |
