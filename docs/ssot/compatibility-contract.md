...
| behavior planning | primary plans cover all 531 public entries | not started | 387 substantive reviewed primary plans with 389 fixed source/interaction projections; 144 explicit M0 gaps remain; 3 support-script and 23 command plans have executable suite bindings; `perl2lcov --preserve` and 7 capture/reset-oriented `lcov` entries remain unbound; all product evidence remains empty |
