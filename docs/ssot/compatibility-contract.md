...
| behavior planning | primary plans cover all 531 public entries | not started | 397 substantive reviewed primary plans with 399 fixed source/interaction projections; 134 explicit M0 gaps remain; 3 support-script, 23 command, and 10 `lcovrc` plans have executable suite bindings; deterministic `genhtml` captures use a fixed `SOURCE_DATE_EPOCH`; all product evidence remains empty |
