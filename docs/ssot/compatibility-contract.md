...
| behavior planning | primary plans cover all 531 public entries | not started | 389 substantive reviewed primary plans with 391 fixed source/interaction projections; 142 explicit M0 gaps remain; 3 support-script, 23 command, and 2 `lcovrc` list-format plans have executable suite bindings; `lcov_list_truncate_max` remains unbound; all product evidence remains empty |
