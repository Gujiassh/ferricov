...
| behavior planning | primary plans cover all 531 public entries | not started | 419 substantive reviewed primary plans with 421 fixed source/interaction projections; 112 explicit M0 gaps remain; 3 support-script, 23 command, and 32 `lcovrc` plans have executable suite bindings; deterministic `genhtml` captures use a fixed `SOURCE_DATE_EPOCH`, report-wave Perl hash seeds, and explicit baseline date; all product evidence remains empty |
