...
fn canonical_writer_orders_families_recomputes_totals_and_is_stable() {
    ...
    let bytes = write_canonical(&database, &context).expect("serializable");
...
fn branch_mcdc_non_utf8_and_round_trip_boundaries_are_canonical() {
    ...
    assert_eq!(
        write_canonical(&reparsed, &SerializationContext::default()).expect("serializable"),
        output
    );
}
...
#[test]
fn numeric_expression_is_preserved_but_absent_expression_fails_closed() {
    let numeric = StreamingParser::parse_database(
        b"TN:t\nSF:numeric.c\nBRDA:1,0,0,1\nDA:1,1\nend_of_record\n",
    );
    let source = numeric.iter().next().unwrap().1;
    let test = source.testcases().branches().values().next().unwrap();
    let edge = test.lines().next().unwrap().1.blocks()[0]
        .edges()
        .first()
        .unwrap();
    assert_eq!(
        edge.expression().map(|value| value.as_bytes()),
        Some(b"0".as_slice())
    );
    ...
    assert!(matches!(
        write_canonical(&absent, &SerializationContext::default()),
        Err(SerializationError::BranchExpressionAbsent { edge_index: 0, .. })
    ));
